...
for (const width of WIDTHS) {
  for (const depth of DEPTHS) {
    ...
    SHELVES.push({
      ...
      sku: nextSku('SHF'),
      type: 'SHELF',
      name: { ru: `Полка ${meta.ru} ${width.value}×${depth.value}`, kk: `${meta.kk} сөре ${width.value}×${depth.value}` },
      sellingPrice,
...
for (const [width, depth] of REINFORCED_COMBOS) {
  ...
  SHELVES.push({
    ...
    sku: nextSku('SHF'),
    type: 'SHELF',
    name: { ru: `Полка ${meta.ru} ${width}×${depth}`, kk: `${meta.kk} сөре ${width}×${depth}` },
    sellingPrice,
    purchasePrice: Math.round(sellingPrice / 1.4),
...
const GENERIC_COMPONENTS: ShelvingComponent[] = [
  ...
  },
  {
    ...
    sku: nextSku('FST'),
    type: 'FASTENER',
    name: { ru: 'Комплект крепежа (болт + гайка)', kk: 'Бекітпе жинағы (бұранда + сомын)' },
    sellingPrice: 120,
    purchasePrice: 70,
    weightKg: 0.05,
    models: [],
...
export const ACCESSORIES: Accessory[] = [
  ...
  },
  {
    ...
    sku: 'ACC-0005',
    slug: 'cross-brace',
    name: { ru: 'Крестовина жёсткости', kk: 'Қатайтқыш айқыш' },
...
export const ASSEMBLY_SERVICES: AssemblyService[] = [
  {
    id: 'assembly-self',
    name: { ru: 'Самостоятельная сборка', kk: 'Өз бетінше құрастыру' },
    description: { ru: 'Стеллаж поставляется в разобранном виде с инструкцией', kk: 'Стеллаж нұсқаулықпен бірге бөлшектелген түрде жеткізіледі' },
    method: 'FIXED',
    value: 0,
    sortOrder: 0,
    active: true,
  },
  {
    ...
    name: { ru: 'Профессиональная сборка', kk: 'Кәсіби құрастыру' },
    description: { ru: 'Сборка бригадой на вашем объекте', kk: 'Нысаныңызда бригадамен құрастыру' },
    method: 'PER_SECTION',
    value: 6000,
    sortOrder: 1,
    active: true,
  },
  {
    ...
    name: { ru: 'Сборка и монтаж «под ключ»', kk: '«Толық аяқталған» форматында құрастыру және монтаждау' },
    description: { ru: 'Сборка, крепление к стене/полу и уборка упаковки', kk: 'Құрастыру, қабырғаға/еденге бекіту және қаптаманы жинап әкету' },
    method: 'PERCENT',
    value: 8,
    sortOrder: 2,
    active: true,
...
export const DELIVERY_METHODS: DeliveryMethod[] = [
  {
    id: 'delivery-pickup',
    kind: 'PICKUP',
    name: { ru: 'Самовывоз со склада', kk: 'Қоймадан алып кету' },
    description: {
  ...
  },
  {
    id: 'delivery-city',
    kind: 'CITY',
    name: { ru: 'Доставка по городу', kk: 'Қала ішінде жеткізу' },
    description: {
  ...
  },
  {
    ...
    name: { ru: 'Передача транспортной компании', kk: 'Көлік компаниясына тапсыру' },
    description: { ru: 'Отгрузка на терминал транспортной компании по вашему выбору', kk: 'Сіз таңдаған көлік компаниясының терминалына жеткізу' },
    basePrice: null,
    requiresAddress: false,
    sortOrder: 3,
    active: true,
